# Load DF-04-03 with an INT8 language model and predict continuous action chunks.
...
# Load and execute the strict mixed-precision LoRA+OFT INT8 inference path.
class LoraOFTINT8Policy:
    # Initialize quantized model artifacts, continuous heads, and diagnostics.
    def __init__(self, config: Mapping[str, Any]) -> None:
        ...
        # Measure the complete GPU model construction, including external heads.
        ...
        self.load_peak_vram_allocated_gib = (
            torch.cuda.max_memory_allocated(self.device) / 1024**3
        )
        self.load_peak_vram_reserved_gib = (
            torch.cuda.max_memory_reserved(self.device) / 1024**3
        )
        self.quantization_audit = build_quantization_audit(
            model=self.model,
            action_head=self.action_head,
            proprio_projector=self.proprio_projector,
            quantization_config=self.quantization_config,
            lora_parameter_names=self.lora_parameter_names,
            model_load_seconds=self.model_load_seconds,
            load_peak_vram_allocated_gib=self.load_peak_vram_allocated_gib,
            load_peak_vram_reserved_gib=self.load_peak_vram_reserved_gib,
        )
        self.quantization_audit["peft_bnb_legacy_state_compatibility"] = {
            "peft_version": "0.11.1",
            "state_field": "memory_efficient_backward",
            "restored_default_value": False,
            "patched_module_count": len(
                self.peft_bnb_compatibility_module_names
            ),
            "patched_module_names": list(
                self.peft_bnb_compatibility_module_names
            ),
        }
        validate_quantization_audit(
        ...
        self.quantization_audit["audit_passed"] = True
    ...
    # Restore legacy bitsandbytes state metadata still read by PEFT 0.11.1.
    @staticmethod
    def _restore_peft_bnb_legacy_state(
        model: torch.nn.Module,
    ) -> Tuple[str, ...]:
        # Modern CUDA-12.8 bitsandbytes removed this unused inference flag.
        ...
        for module_name, module in model.named_modules():
            ...
            if state is None:
                raise RuntimeError(
                    f"INT8 module has no MatmulLtState: {module_name}."
                )
            if not hasattr(state, "memory_efficient_backward"):
                state.memory_efficient_backward = False
                patched_module_names.append(module_name)

        if quantized_module_count <= 0:
            raise RuntimeError(
                "No Linear8bitLt modules existed before PEFT adapter injection."
            )
        return tuple(sorted(patched_module_names))

    # Load an INT8 OpenVLA language model and attach the BF16 LoRA adapter.
    def _load_model(self) -> Tuple[PeftModel, OpenVLAForActionPrediction]:
        ...
        base_model = AutoModelForVision2Seq.from_pretrained(
            ...
            device_map={
                "": int(self.quantization_config["device_index"])
            },
        ...
        self.peft_bnb_compatibility_module_names = (
            self._restore_peft_bnb_legacy_state(base_model)
        )
        ...
        self.lora_parameter_names = cast_lora_parameters(
            model,
            device=self.device,
            dtype=self.dtype,
        )
    ...
    def predict(
        ...
        return action_chunk.copy(), {
            ...
            "inference_path": (
                "continuous_l1_action_head_int8_language_model"
            ),
            "discrete_action_token_decoding_used": False,
            "quantization_backend": self.quantization_config["backend"],
            "quantization_method": self.quantization_config["method"],
            "language_model_loaded_in_8bit": self.quantization_audit[
                "is_loaded_in_8bit"
            ],
