# Start the DF-06-01 LoRA+OFT INT8 closed-loop LIBERO simulation.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run DF-06-01 LoRA+OFT INT8 LIBERO simulation."
    )
    parser.add_argument(
        "--config",
        type=Path,
...
def validate_config(config: Mapping[str, Any]) -> None:
    ...
    if model["policy_type"] != "lora_oft_l1_chunk_int8":
        raise ValueError(
            "model.policy_type must be 'lora_oft_l1_chunk_int8'."
        )
    ...
    if str(model["attn_implementation"]) != "eager":
        raise ValueError("DF-06-01 must use eager attention for fair comparison.")
    if int(model["checkpoint_step"]) != 100000:
        raise ValueError("DF-06-01 must evaluate the step-100000 best adapter.")
    if str(model["torch_dtype"]) != "bfloat16":
        raise ValueError("All non-quantized DF-06-01 components must use BF16.")

    if not bool(quantization["enabled"]):
        raise ValueError("DF-06-01 requires quantization.enabled=true.")
    if str(quantization["backend"]) != "bitsandbytes":
        raise ValueError("DF-06-01 requires the bitsandbytes backend.")
    if str(quantization["method"]) != "llm_int8":
        raise ValueError("DF-06-01 requires the llm_int8 method.")
    if (
    ...
    if not bool(quantization["load_in_8bit"]):
        raise ValueError("quantization.load_in_8bit must be true.")
    if bool(quantization["load_in_4bit"]):
        raise ValueError("quantization.load_in_4bit must be false.")
    if float(quantization["llm_int8_threshold"]) <= 0.0:
        raise ValueError("llm_int8_threshold must be positive.")
    if bool(quantization["llm_int8_enable_fp32_cpu_offload"]):
        raise ValueError("FP32 CPU offload is forbidden for this experiment.")
    if int(quantization["device_index"]) != 0:
        raise ValueError("DF-06-01 must use CUDA device index 0.")
    ...
    if not bool(quantization["require_quantized_linear_modules"]):
        raise ValueError("Actual Linear8bitLt modules must be required.")
    ...
    if not bool(action["stop_on_invalid_action"]):
        raise ValueError("DF-06-01 must stop a rollout after an invalid action.")
    ...
    if simulation["task_suite_name"] != "libero_10":
        raise ValueError("DF-06-01 evaluates the libero_10 task suite.")
...
# Save the hard INT8 model audit produced during policy initialization.
...
def run_preflight(
    ...
    logging.info(
        (
            "DF-06-01 preflight: action_shape=%s latency_ms=%.3f "
            "int8_linear_modules=%d zero_equivalent=%d"
        ),
        tuple(chunk.shape),
...
def main() -> int:
    ...
    try:
        ...
        if args.validate_only:
            print("DF-06-01 config validation: OK")
            return 0
        ...
        if not torch.cuda.is_available():
            raise RuntimeError("DF-06-01 requires a CUDA GPU.")
        ...
        from src.lora_oft_int8_policy import LoraOFTINT8Policy
        from src.simulation_runner import SimulationRunner

        logging.info(
            "Loading DF-04-03 step-100000 adapter with INT8 language model."
        )
        policy = LoraOFTINT8Policy(config)
        ...
        logging.exception("DF-06-01 simulation failed.")
        return 1
