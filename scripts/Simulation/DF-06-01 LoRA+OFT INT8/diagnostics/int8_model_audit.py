# Load the DF-06-01 policy and write a standalone INT8 materialization audit.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Audit the loaded DF-06-01 bitsandbytes INT8 model."
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=PROJECT_ROOT / "config" / "simulation_config.yaml",
        help="Path to the DF-06-01 simulation YAML.",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main() -> int:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("The INT8 model audit requires CUDA.")
    ...
    print("DF-06-01 INT8 model audit: OK")
    print(
