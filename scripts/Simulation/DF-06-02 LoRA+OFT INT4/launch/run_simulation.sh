...
# Launch DF-06-02 with the exact cloud interpreter and NF4 dependencies.
set -euo pipefail

PROJECT_ROOT="/root/autodl-tmp/openvla_efficiency_project/scripts/Simulation/DF-06-02 LoRA+OFT INT4"
...
# Prove that bitsandbytes can execute an NF4 linear kernel on the RTX 5090.
"$PYTHON_BIN" - <<'PY'
...
if outputs.shape != (2, 8) or not torch.isfinite(outputs).all():
    raise RuntimeError("bitsandbytes NF4 kernel smoke test returned invalid output.")

print("DF-06-02 NF4 dependency check: OK")
