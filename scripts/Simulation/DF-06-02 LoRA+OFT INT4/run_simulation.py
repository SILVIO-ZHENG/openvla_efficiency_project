# Start the DF-06-02 LoRA+OFT INT4 closed-loop LIBERO simulation.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run DF-06-02 LoRA+OFT INT4 LIBERO simulation."
    )
    parser.add_argument(
        "--config",
        type=Path,
...
def validate_config(config: Mapping[str, Any]) -> None:
    ...
    if model["policy_type"] != "lora_oft_l1_chunk_nf4":
        raise ValueError(
            "model.policy_type must be 'lora_oft_l1_chunk_nf4'."
        )
    ...
    if str(model["attn_implementation"]) != "eager":
        raise ValueError("DF-06-02 must use eager attention for fair comparison.")
    if int(model["checkpoint_step"]) != 100000:
        raise ValueError("DF-06-02 must evaluate the step-100000 best adapter.")
    if str(model["torch_dtype"]) != "bfloat16":
        raise ValueError("All non-quantized DF-06-02 components must use BF16.")

    if not bool(quantization["enabled"]):
        raise ValueError("DF-06-02 requires quantization.enabled=true.")
    if str(quantization["backend"]) != "bitsandbytes":
        raise ValueError("DF-06-02 requires the bitsandbytes backend.")
    if str(quantization["method"]) != "bnb_4bit_nf4":
        raise ValueError("DF-06-02 requires the bnb_4bit_nf4 method.")
    if (
        str(quantization["quantized_component"])
        != "language_model_linear_layers"
    ):
        raise ValueError("Only language-model linear layers may be quantized.")
    if bool(quantization["load_in_8bit"]):
        raise ValueError("quantization.load_in_8bit must be false.")
    if not bool(quantization["load_in_4bit"]):
        raise ValueError("quantization.load_in_4bit must be true.")
    if str(quantization["bnb_4bit_quant_type"]) != "nf4":
        raise ValueError("bnb_4bit_quant_type must be 'nf4'.")
    if str(quantization["bnb_4bit_compute_dtype"]) != "bfloat16":
        raise ValueError("bnb_4bit_compute_dtype must be 'bfloat16'.")
    if not bool(quantization["bnb_4bit_use_double_quant"]):
        raise ValueError("bnb_4bit_use_double_quant must remain true.")
    if int(quantization["device_index"]) != 0:
        raise ValueError("DF-06-02 must use CUDA device index 0.")
    if str(model["device"]) != "cuda:0":
        raise ValueError("model.device must match quantization device cuda:0.")
    required_skip_modules = {
        "vision_backbone",
        "projector",
        "lm_head",
    }
    if set(quantization["skip_modules"]) != required_skip_modules:
        raise ValueError(
            "skip_modules must contain vision_backbone, projector, and lm_head."
        )
    required_bfloat16_flags = (
        "keep_lora_in_bfloat16",
        "keep_vision_backbone_in_bfloat16",
        "keep_multimodal_projector_in_bfloat16",
        "keep_lm_head_in_bfloat16",
        "keep_action_head_in_bfloat16",
        "keep_proprio_projector_in_bfloat16",
    )
    if not all(bool(quantization[key]) for key in required_bfloat16_flags):
        raise ValueError("Every non-quantized component must remain in BF16.")
    if not bool(quantization["require_quantized_linear_modules"]):
        raise ValueError("Actual Linear4bit modules must be required.")
    if not bool(quantization["forbid_cpu_or_disk_offload"]):
        raise ValueError("CPU and disk offload must remain forbidden.")
    ...
    if not bool(action["stop_on_invalid_action"]):
        raise ValueError("DF-06-02 must stop a rollout after an invalid action.")
    ...
    if simulation["task_suite_name"] != "libero_10":
        raise ValueError("DF-06-02 evaluates the libero_10 task suite.")
...
# Save the hard NF4 model audit produced during policy initialization.
def save_quantization_audit(
    config: Mapping[str, Any],
    policy: Any,
) -> None:
    # Write a standalone report so preflight runs prove actual quantization.
    path = (
...
def run_preflight(
    ...
    logging.info(
        (
            "DF-06-02 preflight: action_shape=%s latency_ms=%.3f "
            "nf4_linear_modules=%d zero_equivalent=%d"
        ),
        tuple(chunk.shape),
        float(info["inference_latency_ms"]),
        int(policy.quantization_audit["quantized_linear_module_count"]),
...
def main() -> int:
    ...
    try:
        ...
        if args.validate_only:
            print("DF-06-02 config validation: OK")
            return 0
        ...
        if not torch.cuda.is_available():
            raise RuntimeError("DF-06-02 requires a CUDA GPU.")
        ...
        logging.info(
            "Loading DF-04-03 step-100000 adapter with NF4 language model."
        )
        policy = LoraOFTINT4Policy(config)
        if bool(config["recording"]["save_quantization_audit"]):
            save_quantization_audit(config, policy)
        ...
        logging.exception("DF-06-02 simulation failed.")
        return 1
