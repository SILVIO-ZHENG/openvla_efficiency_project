# Compare DF-05-02 BF16 and DF-06-02 NF4 structured simulation results.
...
# Parse the BF16 baseline, NF4 result, and output locations.
def parse_args() -> argparse.Namespace:
    # Require the baseline path because cloud and local layouts can differ.
    parser = argparse.ArgumentParser(
        description="Compare DF-05-02 BF16 with DF-06-02 NF4."
    )
    parser.add_argument(
        "--bf16-metrics",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="DF-06-02 output/metrics directory.",
    )
    parser.add_argument(
        "--output-directory",
        type=Path,
...
def compare_value(
    ...
    # Use NF4 minus BF16 so negative latency and VRAM deltas mean reductions.
...
def compare_tasks(
    ...
    if set(baseline_by_task) != set(quantized_by_task):
        raise ValueError("BF16 and NF4 task-summary indices differ.")
        ...
        comparisons.append(
            {
                ...
                "nf4_success_rate": comparison["nf4"],
                "nf4_minus_bf16": comparison["nf4_minus_bf16"],
            }
        )
    return comparisons
...
def write_markdown(
    ...
    for row in aggregate_rows:
        ...
        lines.append(
            "| {metric} | {bf16} | {nf4} | {delta} | {relative} |".format(
                metric=row["metric"],
                bf16=row["bf16"],
                nf4=row["nf4"],
                delta=row["nf4_minus_bf16"],
    ...
    for row in task_rows:
        lines.append(
            ...
            f"{row['nf4_success_rate']} | {row['nf4_minus_bf16']} |"
        )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")


# Build and save all BF16-versus-NF4 comparison artifacts.
def main() -> int:
    # Read only finalized structured records, never parse console logs.
    args = parse_args()
    bf16_metrics = args.bf16_metrics.expanduser().resolve()
    nf4_metrics = args.nf4_metrics.expanduser().resolve()
    output_directory = args.output_directory.expanduser().resolve()
    output_directory.mkdir(parents=True, exist_ok=True)

    baseline = load_json(bf16_metrics / "simulation_summary.json")
    quantized = load_json(nf4_metrics / "simulation_summary.json")
    ...
    task_rows = compare_tasks(
        ...
        load_csv(nf4_metrics / "task_summary.csv"),
    )
    report = {
        "comparison": "DF-05-02 BF16 versus DF-06-02 NF4",
        "bf16_run_id": baseline["run_id"],
        "nf4_run_id": quantized["run_id"],
    ...
    write_json(output_directory / "bf16_vs_nf4_comparison.json", report)
    write_csv(output_directory / "bf16_vs_nf4_comparison.csv", aggregate_rows)
    write_markdown(
        output_directory / "bf16_vs_nf4_comparison.md",
        aggregate_rows,
        task_rows,
    )
    print("DF-05-02 BF16 versus DF-06-02 NF4 comparison: OK")
