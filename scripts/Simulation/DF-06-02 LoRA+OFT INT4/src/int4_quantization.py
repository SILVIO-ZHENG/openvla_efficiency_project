# Build and audit the mixed-precision 4-bit NF4 inference configuration.
...
# Construct the exact Hugging Face bitsandbytes NF4 configuration from YAML.
def build_int4_quantization_config(
    quantization_config: Mapping[str, Any],
) -> BitsAndBytesConfig:
    # Reject alternate methods so this experiment cannot silently become INT8.
    if not bool(quantization_config["enabled"]):
        raise ValueError("4-bit NF4 quantization must remain enabled.")
    if str(quantization_config["backend"]) != "bitsandbytes":
        raise ValueError("quantization.backend must be 'bitsandbytes'.")
    if str(quantization_config["method"]) != "bnb_4bit_nf4":
        raise ValueError("quantization.method must be 'bnb_4bit_nf4'.")
    if bool(quantization_config["load_in_8bit"]):
        raise ValueError("INT8 loading must remain disabled in DF-06-02.")
    if not bool(quantization_config["load_in_4bit"]):
        raise ValueError("quantization.load_in_4bit must be true.")
    if str(quantization_config["bnb_4bit_quant_type"]) != "nf4":
        raise ValueError("bnb_4bit_quant_type must be 'nf4'.")
    if str(quantization_config["bnb_4bit_compute_dtype"]) != "bfloat16":
        raise ValueError("bnb_4bit_compute_dtype must be 'bfloat16'.")
    if not bool(quantization_config["bnb_4bit_use_double_quant"]):
        raise ValueError("Nested 4-bit quantization must remain enabled.")
    ...
    return BitsAndBytesConfig(
        ...
        # Transformers uses this compatibility field for modules left unconverted.
...
# Find the Transformers flag proving that 4-bit loading was requested.
def find_loaded_in_4bit(model: nn.Module) -> bool:
...
def build_quantization_audit(
    ...
    return {
        ...
        "is_loaded_in_4bit": find_loaded_in_4bit(model),
...
def validate_quantization_audit(
    ...
    # Fail before simulation when NF4 was requested but not actually materialized.
    failures: List[str] = []
    if not bool(audit["is_loaded_in_4bit"]):
        failures.append("Transformers is_loaded_in_4bit is false")
    if int(audit["quantized_linear_module_count"]) <= 0:
        failures.append("no bitsandbytes Linear4bit modules were found")
    ...
    if outside_language_model:
        failures.append(
            "4-bit modules exist outside language_model: "
    ...
    if failures:
        raise RuntimeError(
            "4-bit NF4 quantization audit failed: " + "; ".join(failures)
        )
