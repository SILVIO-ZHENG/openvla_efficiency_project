...
sub-datasets used by DF-04-02 LoRA Batch12 and related OpenVLA efficiency
experiments.

The script reads the fixed seed-42 logical split JSON, selects only the
train_episode_indices, applies the project-standard LIBERO action/proprio
standardization, and writes one provenance-rich JSON file:
...
# EN: These paths stay inside the uploadable DF-04-02 folder.
# 中文：这些路径始终位于可整体上传的 DF-04-02 文件夹内部。
...
# 中文：数据集体积过大，因此仍放在 DF-04-02 文件夹外部。
...
# EN: These are the only four modified LIBERO sub-datasets expected for DF-04-02.
...
# 中文：这里固定了 DF-04-02 预期使用的四个 modified LIBERO 子数据集。
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "LIBERO RLDS datasets used by DF-04-02 LoRA+OFT."
        )
    )
    parser.add_argument(
        "--dataset-root",
