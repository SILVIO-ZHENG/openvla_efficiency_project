# =============================================================================
# DF-04-02 LoRA-OneStepCE Dataset Pipeline
# =============================================================================
#
# EN: This file prepares one-step OpenVLA action-token training samples for the
#     project's LoRA baseline rerun.
#
# EN: Main supported features:
#     1. One-step action-token supervision: 1 x 7 action tokens.
...
#
# EN: This file prepares data only. It does not use the OFT continuous action
#     head and does not require eight-step action chunks.
# =============================================================================
...
class RLDSBatchTransform:
    """
    EN: Converts one RLDS sample into one-step OpenVLA action-token training data.
    """
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        """
        EN: Build a one-step action-token sequence and return the associated
            image inputs and optional metadata.
        """
        ...
        # EN: future_action_window_size=0 normally yields shape [1, 7].
        #     Keep a fallback for pipelines that already squeeze to [7].
        if action.shape == (1, ACTION_DIM):
            current_action = action[0]
        elif action.shape == (ACTION_DIM,):
            current_action = action
        else:
            raise ValueError(
                "Expected one-step action shape "
                f"{(1, ACTION_DIM)} or {(ACTION_DIM,)}, but got {action.shape}."
            )
        ...
        # EN: Tokenize only the current 7-D action for OpenVLA one-step CE.
        action_string = self.action_tokenizer(current_action)
        action_token_count = len(action_string)

        expected_action_token_count = ACTION_DIM
        if action_token_count != expected_action_token_count:
            raise ValueError(
                "Unexpected number of action tokens: "
                f"actual={action_token_count}, "
        ...
        # EN: Compute language-model loss only on the 7 action tokens and,
        #     when enabled, the final stop token.
        labels[: -(action_token_count + 1)] = IGNORE_INDEX
        ...
        output: Dict[str, Any] = {
            ...
            # EN: Keep the current action for logging or optional checks.
    ...
    def __init__(
        ...
        rlds_config = {
            "traj_transform_kwargs": {
                ...
                "window_size": 1,
                # EN: One-step LoRA supervises only the current action.
