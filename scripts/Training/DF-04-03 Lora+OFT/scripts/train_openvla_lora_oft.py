"""
DF-04-03 LoRA+OFT portable training wrapper.
...
4. passes LoRA, OFT, split, validation, checkpoint, proprioception,
   normalization, and logging settings to scripts/finetune.py;
5. reads the ACTUAL runtime configuration written by finetune.py;
6. writes reproducible metadata, Markdown, CSV, and a launch-resolved config.
...
TASK_ID = "DF-04-03_LoRA+OFT"
DEFAULT_CONFIG_NAME = "train_config_lora_oft.yaml"
...
def validate_config(config: dict) -> None:
    """Validate the portable LoRA+OFT YAML schema."""
    ...
    if not config["lora"]["use_lora"]:
        raise ValueError("DF-04-03 requires lora.use_lora=true.")
    ...
    if not bool(config["oft"]["use_proprio"]):
        raise ValueError("oft.use_proprio must be true for this run.")
    if not bool(config["oft"]["use_l1_regression"]):
        raise ValueError("oft.use_l1_regression must be true for LoRA+OFT.")
    if bool(config["oft"]["use_diffusion"]):
        raise ValueError("oft.use_diffusion must be false for the current OFT recipe.")
...
def resolve_config(raw_config: dict, config_path: Path) -> dict:
    ...
    paths.setdefault("log_path", str(output_dir / "df_04_03_lora_oft_training.md"))
    paths.setdefault("summary_csv", str(output_dir / "df_04_03_lora_oft_summary.csv"))
    paths.setdefault("metadata_json", str(output_dir / "df_04_03_lora_oft_metadata.json"))
...
def check_source_contract(config: dict) -> dict:
    ...
    dataset_path = source_root / "prismatic/vla/datasets/rlds/dataset.py"
    mixtures_path = source_root / "prismatic/vla/datasets/rlds/oxe/mixtures.py"

    for path in [finetune_path, dataset_path, mixtures_path]:
        if not path.exists():
    ...
    if "OPENVLA_EPISODE_SPLIT_JSON" not in dataset_text:
        raise RuntimeError(
            ...
            "was not found in dataset.py."
        )

    if "train[:95%]" in dataset_text or "train[:95%]" in finetune_text:
        raise RuntimeError(
...
def build_finetune_command(config: dict) -> List[str]:
    ...
    return [
        ...
        # OFT architecture configuration.
        "--use_l1_regression", bool_text(oft["use_l1_regression"]),
        "--use_diffusion", bool_text(oft["use_diffusion"]),
        "--use_film", bool_text(oft["use_film"]),
        "--num_images_in_input", str(oft["num_images_in_input"]),
        "--use_proprio", bool_text(oft["use_proprio"]),
...
def run_lora_oft_training(config: dict) -> Dict[str, Any]:
...
def build_metadata(
    ...
    actual_oft = runtime.get("oft", {}) if isinstance(runtime, dict) else {}
    ...
    return {
        ...
        "requested_oft_config": config["oft"],
        "actual_oft_config": actual_oft,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Portable DF-04-03 LoRA+OFT training wrapper."
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=default_config,
        help="Path to train_config_lora_oft.yaml.",
    )
    parser.add_argument(
        "--preflight-only",
        action="store_true",
    ...
    try:
        ...
        if args.preflight_only:
            ...
            training_result = run_lora_oft_training(config)

            if training_result.get("returncode") != 0:
                raise RuntimeError(
                    "LoRA+OFT finetune.py failed with returncode "
    ...
    print(f"{TASK_ID} LoRA+OFT wrapper finished with status: {status}")
