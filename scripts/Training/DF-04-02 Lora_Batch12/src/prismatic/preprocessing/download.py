...
def download_with_progress(url: str, download_dir: Path, chunk_size_bytes: int = 1024) -> Path:
    ...
    overwatch.info(f"Downloading {(dest_path := download_dirPath(url).name)} from `{url}`", ctx_level=1)
    if dest_path.exists():
        return dest_path
...
def download_extract(dataset_id: str, root_dir: Path) -> None:
    ...
    os.makedirs(download_dir := root_di / r"download" / dataset_id, exist_ok=True)
