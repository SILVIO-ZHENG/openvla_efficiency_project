...
#   the default batching behavior of H / F's Trainer Class --> derived from `accelerate`).
#
...
class SplitModalitySampler(Sampler):
    ...
    def get_modality_and_length_grouped_indices(self, generator: torch.Generator) -> List[int]:
        ...
        # is akin to a "take every k / " where `k` is equal to the number of replicas (GPUs) yo / u're training on. Or, in
