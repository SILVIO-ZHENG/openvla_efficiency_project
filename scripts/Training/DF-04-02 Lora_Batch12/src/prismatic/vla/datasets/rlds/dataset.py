# =============================================================================
# DF-04-03 Dissertation Training Modifications
# =============================================================================
#
# Notes:
...
#
# Purpose:
# The dissertation project uses a fixed seed-42 logical partition:
# train=1480, validation=40, and test=173. Each sub-dataset keeps
# 10 validation episodes, while the original test partition remains unchanged.
# This patch makes the official OpenVLA RLDS loader respect that fixed split
# during LoRA+OFT training.
#
# Author:
# Silvio Zheng
#
...
def make_dataset_from_rlds(
    ...
    *,
    ...
    # Construct the dataset from the fixed episode-level split manifest.
    split_json_path = os.environ.get(
        "OPENVLA_EPISODE_SPLIT_JSON"
    )
    selected_indices_tensor = None
    ...
    if split_json_path:
        ...
        for record in split_data.get(
            "sub_datasets",
            [],
        ):
            if record.get("sub_dataset") == name:
                selected_indices = record.get(
                    f"{split_name}_episode_indices"
                )
        ...
        if (
            ...
            raise ValueError(
                f"No valid {split_name}_episode_indices "
                f"found for dataset {name} in "
                f"{split_json_path}"
            )
        ...
        if any(
            ...
            raise ValueError(
                f"{name}.{split_name}_episode_indices "
                "must contain non-negative integers."
            )

        if len(set(selected_indices)) != len(
            selected_indices
        ):
            raise ValueError(
                f"{name}.{split_name}_episode_indices "
        ...
        if source_split not in builder.info.splits:
            raise ValueError(
                f"Dataset {name} does not contain "
                f"the physical source split "
                f"{source_split!r}."
            )

        selected_indices_tensor = tf.constant(
            selected_indices,
            dtype=tf.int64,
        )
        split = source_split
        ...
        if "val" not in builder.info.splits:
            raise ValueError(
                f"Dataset {name} has no native validation "
                "split. OPENVLA_EPISODE_SPLIT_JSON "
                "must be set."
            )
    ...
    if split_json_path:
        ...
        dataset = dataset.filter(
            lambda episode_index, trajectory:
            selected_indices_lookup.lookup(
                episode_index
            )
            > 0
        )
...
def make_interleaved_dataset(
    ...
    *,
    ...
    for dataset_kwargs, threads, reads in zip(
        ...
        data_kwargs = copy.deepcopy(dataset_kwargs)
        dataset_frame_transform_kwargs = data_kwargs.pop(
            "dataset_frame_transform_kwargs",
            {},
        )
        precomputed_statistics = all_dataset_statistics[data_kwargs["name"]]
        data_kwargs.pop("dataset_statistics", None)
        ...
        dataset, _ = make_dataset_from_rlds(
            **data_kwargs,
            ...
            dataset_statistics=precomputed_statistics,
        )
        source_dataset = dataset.repeat() if train else dataset
