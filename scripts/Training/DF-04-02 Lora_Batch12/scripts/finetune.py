...
#
# This implementation trains OpenVLA with LoRA using token cross-entropy.
# It keeps the requested wrist-image and proprioception inputs, while disabling
# the OFT continuous L1 action head.
# =============================================================================
...
class FinetuneConfig:
    ...
    # Input and action-target values passed by the YAML wrapper.
    training_objective: str
    use_l1_regression: bool
    use_action_head: bool
    use_diffusion: bool
    use_film: bool
...
def validate_config(cfg: FinetuneConfig) -> None:
    ...
    if not cfg.local_files_only:
        raise ValueError("DF-04-02 requires local_files_only=true.")
    resolve_torch_dtype(cfg.torch_dtype)
    if cfg.device != "cuda":
        raise ValueError("DF-04-02 requires device='cuda'.")
    ...
    if not cfg.use_lora:
        raise ValueError("DF-04-02 requires use_lora=true.")
    if str(cfg.training_objective).strip() != "token_cross_entropy":
        raise ValueError("DF-04-02 requires training_objective='token_cross_entropy'.")
    if cfg.use_l1_regression:
        raise ValueError("DF-04-02 requires use_l1_regression=false.")
    if cfg.use_action_head:
        raise ValueError("DF-04-02 requires use_action_head=false.")
    if cfg.use_diffusion:
        raise ValueError("DF-04-02 does not use diffusion.")
    if cfg.use_film:
        raise ValueError("DF-04-02 does not use FiLM.")
    if cfg.num_images_in_input <= 0:
        raise ValueError("num_images_in_input must be positive.")
    if cfg.num_images_in_input < 2:
        raise ValueError("DF-04-02 Batch12 keeps wrist image input, so num_images_in_input must be at least 2.")
    if not cfg.use_proprio:
        raise ValueError("DF-04-02 Batch12 keeps robot state input, so use_proprio must be true.")
    if cfg.use_quantization:
        raise ValueError("DF-04-02 is not a quantized-training run.")
...
# Compute action-token accuracy on supervised one-step action labels.
def compute_action_token_accuracy(
    logits: torch.Tensor,
    labels: torch.Tensor,
) -> Dict[str, torch.Tensor]:
    # OpenVLA logits may include visual-prefix positions before text tokens.
    # Align logits to the trailing text-token region before applying causal shift.
    if logits.ndim != 3:
        raise ValueError(
            f"logits must have shape [B, S, V], got {tuple(logits.shape)}."
        )
    if labels.ndim != 2:
        raise ValueError(
            f"labels must have shape [B, S], got {tuple(labels.shape)}."
        )
    if logits.shape[0] != labels.shape[0]:
        raise ValueError(
            "logits and labels batch sizes differ: "
            f"{logits.shape[0]} vs {labels.shape[0]}."
        )
    if logits.shape[1] < labels.shape[1]:
        raise ValueError(
            "logits sequence length is shorter than labels sequence length: "
            f"{logits.shape[1]} vs {labels.shape[1]}."
        )

    text_offset = logits.shape[1] - labels.shape[1]
    aligned_logits = logits[:, text_offset:, :]

    shifted_logits = aligned_logits[:, :-1, :]
    ...
    if supervised_mask.sum().item() == 0:
        ...
        return {
            "action_token_accuracy": zero,
            "action_token_correct": zero,
            "action_token_total": zero,
        }

    action_token_mask = torch.zeros_like(supervised_mask)
    expected_action_tokens = ONE_STEP_ACTION_CHUNK_LEN * ACTION_DIM

    for batch_index in range(labels.shape[0]):
        positions = supervised_mask[batch_index].nonzero(as_tuple=False).flatten()
        if positions.numel() < expected_action_tokens:
            raise RuntimeError(
                "Not enough supervised action-token positions for sample "
                f"{batch_index}: actual={positions.numel()}, "
                f"expected={expected_action_tokens}."
            )
        selected_positions = positions[-expected_action_tokens:]
        action_token_mask[batch_index, selected_positions] = True

    predicted_token_ids = shifted_logits.argmax(dim=-1)
    correct_tokens = (
        predicted_token_ids.eq(shifted_labels) & action_token_mask
    ).sum(dtype=torch.float32)
    total_tokens = action_token_mask.sum(dtype=torch.float32)
    accuracy = correct_tokens / total_tokens.clamp_min(1.0)

    return {
        "action_token_accuracy": accuracy.detach().float(),
        "action_token_correct": correct_tokens.detach().float(),
        "action_token_total": total_tokens.detach().float(),
    }


def token_cross_entropy_forward_pass(
    vla: DDP,
    ...
    if use_proprio:
        if proprio_projector is None:
            raise RuntimeError("use_proprio=true but proprio_projector is missing.")
        proprio = batch["proprio"].to(
            device=device,
            dtype=runtime_dtype,
            non_blocking=True,
        )
        expected_proprio_shape = (labels.shape[0], PROPRIO_DIM)
    ...
    with torch.autocast(device_type="cuda", dtype=runtime_dtype):
        output = vla(**forward_kwargs)
        loss = output.loss

    if loss is None:
        raise RuntimeError("The model did not return a token cross-entropy loss.")
    if output.logits is None:
        raise RuntimeError("The model did not return logits for accuracy computation.")

    accuracy_metrics = compute_action_token_accuracy(output.logits, labels)
...
def run_validation(
    ...
    with torch.no_grad():
        for batch in val_dataloader:
            _, metrics = token_cross_entropy_forward_pass(
                vla=vla,
                proprio_projector=proprio_projector,
                batch=batch,
                device=device,
    ...
    if (
        "action_token_correct" in averaged
        and "action_token_total" in averaged
    ):
        total_correct = float(averaged["action_token_correct"]) * float(
            global_batch_count
        )
        total_tokens = float(averaged["action_token_total"]) * float(
            global_batch_count
        )
        averaged["action_token_accuracy"] = (
            total_correct / total_tokens
            if total_tokens > 0.0
            else 0.0
        )
        averaged["action_token_correct"] = total_correct
        averaged["action_token_total"] = total_tokens
...
# Atomically save the adapter, optional proprio projector, processor, and statistics.
...
# Load one trainable auxiliary module state dictionary.
...
def build_runtime_resolved_config(
    ...
    if proprio_projector is not None:
        ...
        proprio_total, proprio_trainable = 0, 0
    ...
    return {
        ...
        "action_target": {
            ...
            "use_action_head": cfg.use_action_head,
            "use_diffusion": cfg.use_diffusion,
            "use_film": cfg.use_film,
            "action_dim": ACTION_DIM,
            "num_actions_chunk": ONE_STEP_ACTION_CHUNK_LEN,
        },
        ...
        "parameters": {
            ...
            "total_params": vla_total + proprio_total,
            "trainable_params": vla_trainable + proprio_trainable,
        },
        "software": {
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("DF-04-02 training requires a CUDA GPU.")
    ...
    proprio_projector_model: Optional[nn.Module] = None
    if cfg.use_proprio:
    ...
    proprio_projector = (
        ...
        if proprio_projector_model is not None
        else None
    )

    trainable_modules: List[nn.Module] = [vla]
    if proprio_projector is not None:
        trainable_modules.append(proprio_projector)
    ...
    train_action_token_accuracy_curve: List[Dict[str, Any]] = []
    val_action_token_accuracy_curve: List[Dict[str, Any]] = []
    ...
    if resume_checkpoint is not None:
        ...
        train_action_token_accuracy_curve = list(
            trainer_state.get("train_action_token_accuracy_curve", [])
        )
        val_action_token_accuracy_curve = list(
            trainer_state.get("val_action_token_accuracy_curve", [])
        )
    ...
    collator = PaddedCollatorForActionPrediction(
        ...
        use_proprio=cfg.use_proprio,
        validate_finite_values=cfg.validate_finite_values,
        expected_action_chunk_len=ONE_STEP_ACTION_CHUNK_LEN,
    ...
    recent_metrics = {
        ...
        "action_token_accuracy": deque(maxlen=cfg.grad_accumulation_steps),
    }

    global_step = initial_global_step
    last_train_loss: Optional[float] = None
    last_train_action_token_accuracy: Optional[float] = None
    ...
    try:
        with tqdm.tqdm(
            ...
            for microbatch_index, batch in enumerate(train_dataloader):
                loss, metrics = token_cross_entropy_forward_pass(
                    vla=vla,
                    proprio_projector=proprio_projector,
                    batch=batch,
                    device=device,
                ...
                if "action_token_accuracy" in smoothed_metrics:
                    last_train_action_token_accuracy = smoothed_metrics[
                ...
                if log_due:
                    ...
                    train_row = {
                        ...
                        "train_action_token_accuracy": last_train_action_token_accuracy,
                    ...
                    train_action_token_accuracy_curve.append(
                        {
                            "step": global_step,
                            "train_action_token_accuracy": (
                                last_train_action_token_accuracy
                            ),
                        }
                    )
                ...
                if validation_due:
                    ...
                    validation_row: Dict[str, Any] = {
                        ...
                        "val_action_token_accuracy": validation_metrics.get(
                            "action_token_accuracy"
                        ),
                    ...
                    val_action_token_accuracy_curve.append(
                        {
                            "step": global_step,
                            "val_action_token_accuracy": validation_row[
                                "val_action_token_accuracy"
                            ],
                        }
                    )
                    ...
                    if is_main_process(distributed_state):
                        ...
                        print(
                            ...
                            f"val_action_token_accuracy="
                            f"{validation_row['val_action_token_accuracy']} "
                            f"train_loss={last_train_loss} "
                            f"train_action_token_accuracy="
                            f"{last_train_action_token_accuracy} "
