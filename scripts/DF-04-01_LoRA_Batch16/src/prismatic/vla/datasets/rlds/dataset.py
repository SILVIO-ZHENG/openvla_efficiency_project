...
def make_dataset_from_rlds(
    ...
    *,
    ...
    split_json_path = os.environ.get("OPENVLA_EPISODE_SPLIT_JSON")

    if split_json_path:
        with tf.io.gfile.GFile(split_json_path, "r") as f:
        ...
        for record in split_data.get("sub_datasets", []):
            if record.get("sub_dataset") == name:
                selected_indices = record.get(f"{split_name}_episode_indices")
                source_split = record.get("source_split", "train")
                break

        if selected_indices is None:
            raise ValueError(f"No {split_name}_episode_indices found for dataset {name} in {split_json_path}")
    ...
    if split_json_path:
        selected_indices_lookup = tf.lookup.StaticHashTable(
            tf.lookup.KeyValueTensorInitializer(
                selected_indices_tensor,
                tf.ones_like(selected_indices_tensor, dtype=tf.int64),
            ),
            default_value=tf.constant(0, dtype=tf.int64),
        )

        dataset = dataset.enumerate()
        dataset = dataset.filter(lambda episode_index, trajectory: selected_indices_lookup.lookup(episode_index) > 0)
        dataset = dataset.map(lambda episode_index, trajectory: trajectory, num_parallel_calls=num_parallel_calls)
